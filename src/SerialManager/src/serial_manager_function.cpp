...

{
       ...
       int res;
        ...
        quaternion_2_euler(msg->pose.orientation.x,msg->pose.orientation.y,
            ...
            encode_payload(msg->pose.position.x );
            encode_payload(msg->pose.position.y );
            encode_payload(msg->pose.position.z );
            encode_payload(euler_x);
            encode_payload(euler_y );
            encode_payload(euler_z);

            res=write_to_serial(&serial);
            //if(res < 0) ROS_INFO("WARNING: ERROR WRITE SERIAL "  ); 
}
...
int write_to_serial(int* serial)
{
    ...
    {
        byte_written=write(*serial,&coda_send_seriale.front(), 1);
        //ch=coda_send_seriale.front();
        //if(byte_written < 0 ) return -1;
        //ROS_INFO("WRITTEN %d bytes char= %c ",byte_written , ch );
        ROS_INFO("WRITTEN %d bytes  ",byte_written );
